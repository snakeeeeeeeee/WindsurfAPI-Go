...
- Current cctest/Claude Code failures concentrate on Direct `ApiServerService/GetChatMessage` with Opus 4.7 native tools after tool history is present: requests show `shape_turns=4`, `tool_history=2`, and upstream returns `internal error` before any useful delta.
- Node does not solve this by putting tool history into `chat_message_prompts`. Node's native bridge converts previous assistant tool_calls + tool results into `CortexTrajectoryStep` buffers and passes them as `SendUserCascadeMessageRequest.additional_steps`.
- Correction: the `trajectory_steps` string found in the LS binary is not confirmed to belong to `api_server_pb.GetChatMessageRequest`. For Direct `GetChatMessageRequest`, field 1 is already `metadata`, so attaching trajectory steps to field 1 would corrupt the request.
- Confirmed Direct request fields include `trajectory_reference` at field 15, `cascade_id` at field 16, `prompt_id` at field 17, and `chat_model_uid` at field 21. Current Go Direct previously generated a new metadata session id and prompt id every request and never reused `cascade_id`.
- Root-fix direction now being tested: keep Opus 4.7 tool continuations native, remove the automatic emulated fallback, and reuse stable Direct `session_id/cascade_id` across first-leg tool call and tool-result continuation. If this still fails in a focused smoke, the next candidate is the payload shape of textual tool history vs `chat_message_prompts`, not hidden emulation.
- Follow-up root cause evidence: the LS binary descriptor for Direct `ChatMessagePrompt` exposes native tool history fields: `tool_calls` field 6 (`exa.codeium_common_pb.ChatToolCall`) and `tool_call_id` field 7. `ChatToolCall` carries `id` field 1, `name` field 2, and `arguments_json` field 3.
- Before this fix, Go's Direct native prompt builder only wrote `message_id/source/prompt/safe_for_code_telemetry`; assistant tool calls and tool results were still serialized into `<tool_calls>` / `<tool_result>` XML-ish text. That means the first native tool leg used real Direct tools, but the continuation mixed native `tools` with textual fake history, which matches the observed upstream `internal error` pattern on `shape_turns=4 tool_history=2`.
- Implemented the root-fix direction for Direct `GetChatMessage`: when tool history is present, Go now forces `chat_message_prompts` projection even if `direct.native_chat_prompts=false`, writes assistant tool calls into `ChatMessagePrompt.tool_calls` field 6, writes tool result ids into `tool_call_id` field 7, and removes XML tool history from both prompt-message text and the top-level prompt.
- The stable `session_id/cascade_id` reuse remains useful for continuity, but by itself it was not sufficient. The request payload must also stop duplicating tool history as plain text and native fields.
