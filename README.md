...
`reasoning_effort` / `output_config.effort` routes Opus 4.7 requests to Windsurf's low/medium/high/xhigh/max UIDs. It does not inject local "thinking" prompt text into the upstream request. Direct tool mode defaults to `native`, which keeps the upstream Claude request shape closest to Windsurf and avoids injecting local tool protocol text into model-fingerprint or conformance tests. If Opus 4.7 receives first-turn `tools` with `tool_choice=auto` and upstream rejects native tools with an internal error, Go retries once as a plain native text request and records it in `/debug/direct` as `tool_fallbacks`. Tool-history continuations no longer auto-switch to emulated mode; they stay native and reuse the same Direct conversation identifiers so upstream failures remain visible instead of being hidden by a text-tool fallback. If a Claude Code/Cline workload explicitly needs the local text-tool protocol, switch `direct.tool_mode` to `auto` or `emulated` for that deployment.
