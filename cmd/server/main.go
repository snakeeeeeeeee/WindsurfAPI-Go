...
func main() {
	...
	mux.Handle("/v1/chat/completions", auth(handler.ChatCompletionsHandler(cfg, accountMgr, directClient, reusePool, modelAccessMgr, proxyMgr, virtualUsageMgr, runtimeConfigMgr)))
	mux.Handle("/v1/messages", auth(handler.MessagesHandler(accountMgr, directClient, reusePool, modelAccessMgr, proxyMgr, virtualUsageMgr, runtimeConfigMgr)))
	mux.Handle("/v1/responses", auth(handler.ResponsesHandler(accountMgr, directClient, reusePool, modelAccessMgr, proxyMgr, virtualUsageMgr, runtimeConfigMgr)))
	mux.Handle("/v1/response", auth(handler.ResponsesHandler(accountMgr, directClient, reusePool, modelAccessMgr, proxyMgr, virtualUsageMgr, runtimeConfigMgr)))
